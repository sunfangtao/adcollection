...
public class InstrumentPanelView extends View {
	 ...
	 * 默认最大刻度
	 */
	public static final double DEFAULT_MAX_SPEED = 220.0;
	/**
	 * 默认的刻度间隔
	 */
	 ...
	 * 刻度默认颜色
	 */
	private int defaultColor = Color.parseColor("#23e23e");
	/**
	 * 刻度间隔（度数）
	 */
	 ...
	 * 可用的角度范围（居中）
	 */
	private float availableAngle = 160 + 2 * angle;
	/**
	 * 刻度所在位置
	 */
	...
	public void setMaxSpeed(double maxSpeed) {
		if (maxSpeed <= 0)
			throw new IllegalArgumentException(
					"Non-positive value specified as max speed.");
	...
	@TargetApi(11)
	public ValueAnimator setSpeed(double progress, long duration,
			long startDelay) {
		if (progress <= 0)
			throw new IllegalArgumentException(
					"Non-positive value specified as a speed.");
		...
		ValueAnimator va = ValueAnimator.ofObject(new TypeEvaluator<Double>() {
			@Override
			public Double evaluate(float fraction, Double startValue,
					Double endValue) {
	...
	public void setMajorTickStep(double majorTickStep) {
		if (majorTickStep <= 0)
			throw new IllegalArgumentException(
					"Non-positive value specified as a major tick step.");
	...
	public void setLabelTextSize(float labelTextSize) {
		this.labelTextSize = labelTextSize * screen_density;
		if (txtPaint != null) {
	...
	private void drawNeedle(Canvas canvas) {
		...
		float angle = (180 - availableAngle) / 2
				+ (float) (getSpeed() / getMaxSpeed() * availableAngle);
		for (int i = 0; i < 11; i++) {
			int changeAngle = i - 5;
			canvas.drawLine(
	...
	private RectF getOval(Canvas canvas, float factor) {
		...
		oval.offset(canvasWidth * (1 - factor) / 2, canvasWidth * (1 - factor)
				/ 2);
		return oval;
	}
	...
	private void init() {
		...
		labelTextSize = 10f * screen_density;
		txtPaint.setTextSize(labelTextSize);
		txtPaint.setTextAlign(Paint.Align.CENTER);
